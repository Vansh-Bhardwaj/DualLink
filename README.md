...
The Details drawer explains filter readiness, route quality, and whether active sessions have actually used both connections. **Check connections** verifies each selected route, DNS, route independence, and application filtering in plain language. Technical activity remains hidden until requested. The default browser is detected from Windows settings and added automatically. Custom applications are matched by their full executable path so another program with the same filename is not selected accidentally; adding the same executable again selects its existing row, and custom rows can be removed directly.
